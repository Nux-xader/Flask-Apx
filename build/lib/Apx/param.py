...
from Apx.exceptions import (
    MissingParam, UnSupported, InvalidValue, InvalidRequest
)
...
class Param:
    ...
    def missing_param(self, name:str):
        raise MissingParam(f"param {name} is require")

    def invalid_param_value(self, name:str, msg:str):
        raise InvalidRequest(f"param {name} must be {msg}")
    ...
    def dump(self, method:str):
        if method == "GET":
            ...
            if self.content_type is None:
                ...
                if isinstance(content_type, str): content_type = content_type.lower()
    ...
    def text(
        ...
        """
        if default_val is not None and not is_require:
            raise InvalidValue("default value can't set if is_require=False")
